...
TICKET_CATEGORY_ID = 123456789012345678          # Category where ticket channels are created
STAFF_ROLE_ID = 123456789012345678              # Role that can see tickets
LOG_CHANNEL_ID = 123456789012345678             # Channel for ticket logs
TICKET_PANEL_CHANNEL_ID = 123456789012345678    # Channel where the "Create Ticket" button will be posted
...
async def on_ready():
    ...
    # Optionally send the ticket panel button on startup
    channel = bot.get_channel(TICKET_PANEL_CHANNEL_ID)
    if channel:
        # Clear previous messages to avoid button spam (optional)
...
async def create_ticket(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description=f"Welcome {member.mention}! Please describe your issue. Staff will assist you shortly.",
...
async def close(ctx):
    """Close the current ticket channel and save transcript."""
